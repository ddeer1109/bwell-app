...
ssh "$ssh_server" -p 10193  "mkdir -p $ssh_key_path"
scp -P 10193 ~/.ssh/"$ssh_key_file" "$ssh_server:$ssh_key_path"
scp -P 10193 ~/.ssh/"$ssh_key_file.pub" "$ssh_server:$ssh_key_path"

# Add the public key to authorized_keys on the server
ssh "$ssh_server" -p 10193 "cat $ssh_key_path/$ssh_key_file.pub >> $ssh_key_path/authorized_keys"